(() => {
    ...
    function rangeInit() {
        const sliderPage = document.getElementById("range-page");
        const sliderPopup = document.getElementById("range-popup");
        const inputPageFrom = document.getElementById("price-from");
        const inputPageTo = document.getElementById("price-to");
        const inputPopupFrom = document.getElementById("popup-price-from");
        ...
        if (sliderPage && sliderPopup) {
            ...
            let isSyncing = false;
            function syncSliders(source, target, inputFrom, inputTo, otherInputFrom, otherInputTo) {
                source.noUiSlider.on("update", (function(values) {
                    if (isSyncing) return;
                    isSyncing = true;
                    ...
                    otherInputFrom.value = from;
                    otherInputTo.value = to;
                    isSyncing = false;
                }));
            }
            syncSliders(sliderPage, sliderPopup, inputPageFrom, inputPageTo, inputPopupFrom, inputPopupTo);
            syncSliders(sliderPopup, sliderPage, inputPopupFrom, inputPopupTo, inputPageFrom, inputPageTo);
            function setupInputSync(inputFrom, inputTo, slider) {
                inputFrom.addEventListener("change", (function() {
                    const currentValues = slider.noUiSlider.get();
                    slider.noUiSlider.set([ this.value, currentValues[1] ]);
                }));
                inputTo.addEventListener("change", (function() {
                    const currentValues = slider.noUiSlider.get();
                    slider.noUiSlider.set([ currentValues[0], this.value ]);
                }));
            }
            setupInputSync(inputPageFrom, inputPageTo, sliderPage);
            setupInputSync(inputPopupFrom, inputPopupTo, sliderPopup);
        }
    }
    rangeInit();
